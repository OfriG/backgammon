...
export function selecting(
  ...
  // עדכון המצב אם כל החיילים הגיעו לאזור הסיום
  thisTurn.turnPlayer.inTheEnd = checkIfInEndZone(thisTurn.turnPlayer, game.board);
  ...
  if (
    ...
    if (!thisTurn.turnPlayer.inTheEnd && readyToEnd(game, thisTurn)) {
      ...
      & start putting pieces out.`, toastStyle(thisTurn));
    }

    if (thisTurn.maxMoves === 0) {
      if (!(thisTurn.turnPlayer instanceof AIPlayer)) {
...
function checkIfInEndZone(player: Player, board: string[][]): boolean {
  const playerName = player.name;
  const playerPieces = board.flatMap((bar, i) =>
    bar.filter(p => p === playerName).map(() => i)
  );

  if (player.name === "White") {
    return playerPieces.every(i => i >= 18 && i <= 23);
  } else {
    return playerPieces.every(i => i >= 0 && i <= 5);
  }
}
